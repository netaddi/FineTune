...
- **Audio Unit effects** — Load AU effects on an app or output device, manage presets, bypass chains, and open custom or generic plugin interfaces
  ...
  - For a stream-specific multichannel tap, FineTune extracts the source device's preferred L/R pair, runs the stereo EQ/AU chain once, and places the result on the destination device's preferred L/R pair; all other native channels pass through
